...
def get_part_color(n_parts):
    colormap = ('red', 'blue', 'yellow', 'magenta', 'green', 'indigo', 'darkorange', 'cyan', 'pink', 'yellowgreen',
                'rosybrown', 'coral', 'chocolate', 'bisque', 'gold', 'yellowgreen', 'aquamarine', 'deepskyblue', 'navy',
                'orchid',
                'maroon', 'sienna', 'olive', 'lightgreen', 'teal', 'steelblue', 'slateblue', 'darkviolet', 'fuchsia',
                'crimson',
                'honeydew', 'thistle',
                ...
                for folder_names in os.listdir(os.path.join(data_root, 'S{}'.format(subject_index))):
                    if folder_names.startswith(action):
                        for camera in os.listdir(
                                os.path.join(data_root, 'S{}'.format(subject_index), folder_names, "imageSequence")):
                            for frame_index in os.listdir(
                                    os.path.join(data_root, 'S{}'.format(subject_index), folder_names, "imageSequence",
                                                 camera)):
                                self.samples.append(
                                    (subject_index, folder_names, camera, int(frame_index.split('.')[0].split("_")[1])))

    ...
    def forward_img(self, index):
        ...
        if self.unsup_mask:
            if self.split == 'train':
                ...
                mask = TF.resize(torch.from_numpy(np.array(
                    Image.open(osp.join(self.pmask_dir, str(data.rel_path).replace('.jpg', '.png'))))).unsqueeze(0),
                                 img.shape[:2], interpolation=InterpolationMode.NEAREST).squeeze(0).numpy() / 255.
        else:
            mask = data.mask
